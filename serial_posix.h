...
//   1. XLOAD_PORT environment variable  (e.g. export
//   XLOAD_PORT=/dev/tty.usbserial-ABC)
...
// the kernel accepts it, then immediately re-apply the real rate via
// IOSSIOSPEED.
...
static std::string _ftposix_find_port(char channel_hint = '\0') {
  ...
  const char *patterns[] = {"/dev/cu.usbserial-*", "/dev/cu.usbmodem*",
  ...
  for (int i = 0; patterns[i]; ++i) {
    ...
    if (glob(patterns[i], 0, nullptr, &g) == 0 && g.gl_pathc > 0) {
      ...
      size_t idx =
          (want_index < (int)g.gl_pathc) ? (size_t)want_index : g.gl_pathc - 1;
...
static FT_STATUS FT_OpenEx(PVOID desc, int /*flags*/, FT_HANDLE *handle) {
  ...
  if (port.empty()) {
    fprintf(stderr, "[serial_posix] No serial port found.\n"
                    "  Connect the device or set: export "
                    "XLOAD_PORT=/dev/tty.usbserial-XXXX\n");
...
static FT_STATUS FT_SetDataCharacteristics(FT_HANDLE handle, int /*bits*/,
...
//   N ms                  → wait up to N ms        (VMIN=0, VTIME in 100 ms
//   units)
...
static FT_STATUS FT_SetTimeouts(FT_HANDLE handle, DWORD read_timeout_ms,
...
static FT_STATUS FT_SetUSBParameters(FT_HANDLE /*handle*/, DWORD /*in_size*/,
                                     DWORD /*out_size*/) {
  return FT_OK;
}
...
static FT_STATUS FT_Write(FT_HANDLE handle, void *buf, DWORD count,
                          DWORD *written) {
  ssize_t total = 0;
...
static FT_STATUS FT_Read(FT_HANDLE handle, void *buf, DWORD count,
                         DWORD *bytes_read) {
  ssize_t total = 0;
